# ONE-SHOT BUILD GUIDE
## Explainable Anomaly Detection for Financial Audits
### For AI Agents — Complete, Unambiguous, Production-Grade

---

## AGENT INSTRUCTIONS — READ FIRST

You are building a complete Python project end-to-end. Follow every instruction
exactly. Do not improvise architecture. Do not split into multiple modules unless
explicitly told to. Do not add features not listed here.

**Build order:** Create files in the exact order listed. After creating each file,
verify it exists before moving to the next step.

**The single most important rule:** Everything lives in ONE file called `app.py`.
Streamlit re-executes the entire script on every user interaction. Splitting into
modules causes import failures and session state bugs. Do not create `utils.py`,
`model.py`, `preprocessing.py`, or any other helper module.

**Hard constraints — never violate these:**
- NEVER call `plt.show()` — it blocks Streamlit's thread and crashes the app
- NEVER call `scaler.fit_transform(X)` on the full dataset — always split first
- NEVER report accuracy as a metric — the dataset is 99.83% imbalanced
- NEVER pass full `X_train_scaled` to `shap.TreeExplainer` — use 100 background samples
- ALWAYS call `plt.close()` after every `st.pyplot(fig)` call
- ALWAYS put `matplotlib.use('Agg')` before importing pyplot
...
1. Accepts the Kaggle Credit Card Fraud CSV as upload
2. Runs Exploratory Data Analysis
3. Trains an Isolation Forest anomaly detection model
4. Explains every flagged transaction using SHAP TreeExplainer (waterfall plots)
5. Shows global feature importance (beeswarm/bar chart)
6. Compares ML model against a rule-based baseline
7. Exports an audit-ready CSV report with metadata header

**Why SHAP not Z-Score:**
Z-score measures data distribution deviation — it does NOT explain the model's
decision. SHAP TreeExplainer computes each feature's exact contribution to the
Isolation Forest anomaly score. These are fundamentally different things.
`shap.TreeExplainer(iso_forest)` is officially supported and tested in the SHAP
library's own test suite. Multiple 2024–2025 peer-reviewed papers use this exact
combination for financial fraud detection.
...
Create exactly these files and folders. Nothing else.
...
├── data/
│   └── .gitkeep                    ← empty placeholder (creditcard.csv added manually)
└── outputs/
    └── .gitkeep                    ← reports saved here at runtime
```
...
## SECTION 6 — app.py (COMPLETE FILE)

Copy this file exactly. Do not rename functions. Do not reorganize sections.
Every section boundary is marked with `# ===` comments — keep them.
...
## SECTION 7 — SPRINT EXIT CONDITIONS

Verify each sprint before proceeding.

### Sprint 1 Exit — Environment + File Structure
- [ ] `python --version` prints 3.12.x
- [ ] `pip install -r requirements.txt` completes with no errors
- [ ] `python -c "import shap; print(shap.__version__)"` prints 0.45+
- [ ] `streamlit run app.py` opens browser at localhost:8501
- [ ] Tab 1 shows file uploader with scope banner
- [ ] All 6 tabs are visible in the navigation bar

### Sprint 2 Exit — Data Upload Works
- [ ] Uploading `creditcard.csv` shows success message
- [ ] Metrics show: 284,807 total, 492 fraud, 0.172% rate
- [ ] First 10 rows preview renders
- [ ] Uploading a `.txt` file shows: "Only .csv files are accepted."
- [ ] Uploading a CSV missing 'Amount' shows the missing columns error

### Sprint 3 Exit — EDA Dashboard
- [ ] Tab 2 shows class distribution bar chart (green=Normal, red=Fraud)
- [ ] Red warning about accuracy metric is visible
- [ ] Amount distribution plots render (2 side by side)
- [ ] Hour of day plot renders
- [ ] Correlation heatmap renders
- [ ] Descriptive statistics table renders

### Sprint 4 Exit — Detection Runs
- [ ] Contamination slider is visible in sidebar
- [ ] Contamination notice warning is visible
- [ ] Clicking "Run Detection" shows progress bar with 4 steps
- [ ] Detection completes without error (may take 60–90 seconds on first run)
- [ ] Success message shows number of anomalies flagged
- [ ] No `plt.show()` calls anywhere — only `st.pyplot(fig)` + `plt.close()`

### Sprint 5 Exit — Results + SHAP
- [ ] Tab 4 shows comparison table: IF vs Rule-Based with P/R/F1 values
- [ ] "Why no Accuracy" error box is visible
- [ ] Confusion matrix 4-metric row renders (TP/FP/TN/FN)
- [ ] Contamination notice warning is visible
- [ ] Global SHAP bar chart renders (top 15 features)
- [ ] Score distribution histogram renders with threshold line

### Sprint 6 Exit — Explorer + Explanation + Export
- [ ] Tab 5 shows sortable table of flagged transactions
- [ ] Selecting a transaction from dropdown persists to session state
- [ ] "Generate CSV Report" button downloads a CSV with metadata header
- [ ] Tab 6 shows 3 metric cards (score, label, amount)
- [ ] SHAP waterfall plot renders for selected transaction
- [ ] Top 5 features listed with SHAP values and direction
- [ ] Red disclaimer box visible at bottom of explanation

---

## SECTION 8 — KNOWN AGENT FAILURE POINTS

These are the exact places agents hallucinate wrong code. Check these explicitly.

### Failure 1: matplotlib in Streamlit
```python
# WRONG — agent often writes this, causes app to freeze
...
st.pyplot(fig)
plt.close()  # mandatory — frees memory, prevents leakage
```

### Failure 2: SHAP background
```python
# WRONG — 227,000 rows as background = 10+ minute hang
explainer = shap.TreeExplainer(model, data=X_train_scaled)

# CORRECT — 100 samples is sufficient
background = shap.sample(
    pd.DataFrame(X_train_scaled, columns=feature_names), 100, random_state=42
)
explainer = shap.TreeExplainer(model, data=background, feature_perturbation='interventional')
```

### Failure 3: SHAP values shape
```python
# Agents miss that TreeExplainer on IF can return a list
shap_values = explainer.shap_values(X_test_scaled)

# Handle both cases
if isinstance(shap_values, list):
    shap_values = shap_values[0]  # always normalize to 2D array
```

### Failure 4: SHAP expected_value
```python
# Agents assume scalar — it may be array
ev = explainer.expected_value
if isinstance(ev, (list, np.ndarray)):
    ev = float(ev[0])  # always cast to scalar for waterfall
```

### Failure 5: Data leakage in scaling
```python
...
# CORRECT — split first, fit only on train
X_train, X_test = train_test_split(X, stratify=y)
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled  = scaler.transform(X_test)  # transform only, no fit
```

### Failure 6: IF label conversion
```python
# IF outputs: -1 = anomaly, +1 = normal
# Ground truth: 1 = fraud, 0 = normal
# Agents often forget this conversion

# CORRECT
y_pred = (labels == -1).astype(int)  # -1 → 1 (fraud), +1 → 0 (normal)
```

### Failure 7: Accuracy metric
```python
# NEVER DO THIS
from sklearn.metrics import accuracy_score
print(accuracy_score(y_test, y_pred))  # will print ~0.998, completely useless
...
### Failure 8: Session state not initialized
```python
# WRONG — crashes if key not yet set
model = st.session_state['model']

# CORRECT — initialize all keys at startup
def init_session_state():
    defaults = {'model': None, 'shap_values': None, ...}
    for key, val in defaults.items():
        if key not in st.session_state:
            st.session_state[key] = val
```
...
## SECTION 9 — EVALUATION METRICS REFERENCE
...
| **AUC-PR** | Area under P-R curve | Better than AUC-ROC for imbalanced data |

Expected benchmarks on Kaggle CC Fraud dataset with 100 trees, contamination=0.001:
- Precision ≈ 0.81
- Recall ≈ 0.71
- F1 ≈ 0.76
- AUC-ROC ≈ 0.97

Source: Performance Analysis of Isolation Forest Algorithm in Fraud Detection of
Credit Card Transactions (ResearchGate, 2022)
...
1. Liu, F.T., Ting, K.M., & Zhou, Z.H. (2008). Isolation forest. *ICDM 2008*, 413–422.
2. Lundberg, S.M., & Lee, S.I. (2017). A unified approach to interpreting model predictions. *NeurIPS 30*.
3. Lässig, F., & Dumonal, M. (2024). A guide to building a financial transaction anomaly detector. *Unit8 Technical Blog*.
4. Latha, N.R., et al. (2025). An XAI-powered approach for financial fraud detection. *JISEM*, 10(2).
5. Ramavath, S.K. (2025). SHAP-enhanced isolation forests for anomaly detection. *IJBAS*, 14(8), 551–561.
6. Herreros-Martínez, A., et al. (2025). Applied ML to anomaly detection in enterprise purchase processes. *MDPI Information*, 16(3), 177.
7. Chandola, V., Banerjee, A., & Kumar, V. (2009). Anomaly detection: A survey. *ACM Computing Surveys*, 41(3).
8. EU GDPR Article 22. (2016). Automated individual decision-making. Official Journal of the EU.

---

## SECTION 11 — HOW TO HAND THIS TO AN AI AGENT

### If using Gemini Antigravity / Google AI Studio:

Paste the following as your task prompt, then attach this entire document:

> "You are a senior Python developer. Build the complete project described in the
> attached BUILD.md file. Create all files listed in Section 2 exactly as specified.
> The entire application must live in a single app.py file — do not split into modules.
> Follow every constraint in Section 8 (Known Agent Failure Points) explicitly.
> After creating each file, confirm it exists. Build in this order:
> requirements.txt → .gitignore → README.md → app.py.
> Do not run or test the app — just create the files."

### If building sprint by sprint (recommended):

Tell the agent one sprint at a time:

**Sprint 1 prompt:**
> "From the BUILD.md guide, complete Sprint 1 only.
> Create: requirements.txt, .gitignore, README.md, data/.gitkeep, outputs/.gitkeep,
> and app.py with only the skeleton (page config, init_session_state, 6 empty tab screens,
> and main()). Verify `streamlit run app.py` opens without errors."

**Sprint 2 prompt:**
> "Sprint 1 is done and verified. Now add Sprint 2 to app.py:
> implement screen_upload() with full validation, file type check,
> and the 4 metric display cards. Follow the code in Section 6 exactly."

Continue sprint by sprint. This prevents the agent from generating
contradictory code across sprints.

---

*Built from: Liu et al. (2008), Lundberg & Lee (2017), Unit8 (2024), JISEM (2025),
MDPI Information (2025), IJBAS (2025), scikit-learn docs, SHAP docs.*
